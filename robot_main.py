...
# ==============================================================
# Camera: Picamera2 NoIR (OV5647) la 640x480, AWB off, ColourGains (1.0, 2.5)
# Tenta IR: NEFOLOSIT (modelul YOLO functioneaza cu raw + colour gains warm)
IR_TINT_R_GAIN = 0.60
IR_TINT_G_GAIN = 1.00
IR_TINT_B_GAIN = 1.15
IR_TINT_ENABLED = False
...
TURN_FACTOR       = 0.6    # legacy, nefolosit
...
STEERING_SAFE_LEFT  = 30.0   # viraj maxim stanga utilizabil
STEERING_SAFE_RIGHT = 90.0   # viraj maxim dreapta utilizabil
...
def make_balls_greener(frame_bgr):
    """NEFOLOSIT (legacy). Era folosit pentru pre-procesare imagine cand
    modelul YOLO necesita accent pe verde. Modelul actual lucreaza pe raw."""
    hsv = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2HSV)
    h, s, v = cv2.split(hsv)

    mask = cv2.inRange(
        hsv,
        np.array((28, 20, 30), dtype=np.uint8),
        np.array((90, 255, 220), dtype=np.uint8),
    )
    ...
    h2[mask > 0] = 58
    s2[mask > 0] = np.clip(
        s2[mask > 0].astype(np.float32) * 2.0 + 45, 0, 255
    ).astype(np.uint8)
    v2[mask > 0] = np.clip(
        v2[mask > 0].astype(np.float32) * 1.25 + 20, 0, 255
    ).astype(np.uint8)

    green = cv2.cvtColor(cv2.merge((h2, s2, v2)), cv2.COLOR_HSV2BGR)
    return cv2.addWeighted(frame_bgr, 0.30, green, 0.70, 0)
...
def init_camera():
    ...
    picam2.set_controls({
        ...
        "AwbEnable":  False,           # AWB off -- ColourGains manual
        "ColourGains": (1.0, 2.5),     # B neutru, R cald (tenta optima YOLO)
        "Brightness": 0.2,
        "Contrast":   1.3,
        "Saturation": 1.1,
    })
...
def apply_sharpening(frame):
    """NEFOLOSIT. Sharpening kernel pe frame -- pipeline-ul curent foloseste raw."""
    return cv2.filter2D(frame, -1, SHARPENING_KERNEL)


def correct_ir_tint(frame_bgr):
    """NEFOLOSIT (IR_TINT_ENABLED=False)."""
    if not IR_TINT_ENABLED:
        return frame_bgr
...
def fix_noir_colors(frame_bgr):
    """NEFOLOSIT. Corecta dominanta rosie a camerei NoIR (B*1.15, R*0.60).
    Pipeline-ul curent foloseste raw + ColourGains=(1.0, 2.5) -- modelul
    YOLO custom recunoaste mingile cu tenta calda."""
    f = frame_bgr.astype(np.float32)
    f[:, :, 0] *= 1.15
    f[:, :, 1] *= 1.00
    f[:, :, 2] *= 0.60
            ...
            if _mpu is not None:
                ...
                if check_stuck(drive_to_waypoint._stuck_st, suspect_threshold=4.0):
                    ...
                    if drive_to_waypoint._stuck_dir > 0:
                        ...
                        set_steering_angle(STEERING_RIGHT_ANGLE - 5)
                    print("[Patrula] Mers inapoi pana 1.5m...")
                    _reverse_50cm()
...
def main():
    ...
    cx0 = 289.0     # centrul mecanic al colectorului (calibrat empiric)
    ...
    def collect_balls(balls_found, patrol_anchor_x=None, patrol_anchor_y=None):
        ...
        for target in path:
            ...
            if distance_z <= STOP_DISTANCE_M:
                ...
                print(f"[{kind.upper()}] Confirmata YOLO -- merg 2.5s")
            ...
            while time.monotonic() - t0_blind < travel_blind:
                ...
                if _mpu is not None:
                    ...
                    if check_stuck(collect_balls._stuck_st_blind, suspect_threshold=8.0):
        ...
        # Acum facem scan nou pentru a vedea daca mai sunt mingi prin apropiere
        # (poate au aparut altele, sau nu am vazut tot la scanul initial).
        stop_and_center()
        time.sleep(0.3)
        print("[Path complet] Scanez din nou dupa mingi ramase...")
        nearby_balls = camera_scan_arc(picam2, model, sports_ball_cls_id, fx, fy, cx0, cy0, patrol)
        if nearby_balls:
            print(f"[Path complet] {len(nearby_balls)} minge(i) noua(i) -- colectez")
            collect_balls(nearby_balls, patrol_anchor_x, patrol_anchor_y)
