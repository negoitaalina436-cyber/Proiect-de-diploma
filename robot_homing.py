...
LED_HSV_LOW         = ( 35,  50, 150)   # verde saturat
LED_HSV_HIGH        = ( 90, 255, 255)

# Estimare distanta la statie din bbox-ul blob-ului LED-urilor.
# STATION_WIDTH_M e o valoare "aparenta", LED-urile saturate apar mai
# mari decat distanta fizica reala intre ele. Calibrat empiric.
STATION_WIDTH_M       = 0.35      # calibrat empiric (2m real vs 1.03m masurat)
...
ROTATE_SEARCH_S      = 3.75    # calibrat pentru ~90deg la HOMING_DUTY_TURN=65%
...
class HomingController:
    ...
    def _detect_led_blob(self, debug=False):
        ...
        try:
            ...
            # Folosim direct frame-ul cu setarile din init_camera (AWB on, brightness -0.3)
    ...
    def home(self):
        """
        Naviga catre statie prin state machine FAR - NEAR.
                            ...
                            if abs(err_deg) > 5.0:
                                ...
                                if err_deg > 0:
                                    self.hw["set_steering_angle"](
                                        self.hw.get("STEERING_SAFE_RIGHT", 85.0))
                                else:
                                    self.hw["set_steering_angle"](
                                        self.hw.get("STEERING_SAFE_LEFT", 43.0))
                                time.sleep(0.3)
    ...
    def _confirm_arrival(self):
        """Verificare finala dupa drive blind, publica AJUNS pe MQTT."""
        self.hw["set_motor_pwm"]("stop", 0.0)
        self.hw["set_steering_angle"](self.hw["STEERING_CENTER_ANGLE"])
        time.sleep(0.5)
...
if __name__ == "__main__":
    ...
    STEERING_LEFT_ANGLE   = 43.0
    STEERING_RIGHT_ANGLE  = 85.0
    SONAR_TRIG_PIN = 18
    SONAR_ECHO_PIN = 16
    SONAR_STOP_M   = 0.50
