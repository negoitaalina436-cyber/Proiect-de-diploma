...
// --- CONFIGURARE HARDWARE STAȚIE ---
const int PIN_LEDURI_IR = 4; // Pinul GPIO4 pentru controlul LED-urilor IR

WiFiClient espClient;
PubSubClient client(espClient);

// --- FUNCȚIA DE ASCULTARE ȘI REACȚIE ---
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (mesaj == "INTOARCERE") {
    ...
    // 1. APRINDEREA FIZICĂ A LED-URILOR IR
    digitalWrite(PIN_LEDURI_IR, HIGH);
    Serial.println(">>> [HARDWARE] LED-urile IR de pe pinul 4 au fost APRINSE!");
  ...
  if (mesaj == "SISTEM_PORNIT") {
    digitalWrite(PIN_LEDURI_IR, LOW);
    Serial.println(">>> [HARDWARE] Resetare: LED-urile IR au fost STINSE.");
  }
}
...
void setup() {
  ...
  pinMode(PIN_LEDURI_IR, OUTPUT);
  digitalWrite(PIN_LEDURI_IR, LOW); // Ne asigurăm că pornesc STINSE la boot

  // --- REȚETA MĂGICĂ PENTRU IPHONE ---
